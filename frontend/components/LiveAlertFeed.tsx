  ...
  return (
    <aside className="w-80 border-r border-radar-border bg-radar-dark flex flex-col z-20 shrink-0">
      <div className="p-4 border-b border-radar-border flex justify-between items-center bg-radar-panel/50">
        <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-radar-cyan flex items-center gap-2">
          <span className="w-2 h-2 bg-radar-cyan rounded-full animate-pulse"></span>
          Diagnostic Feed
        </h2>
        ...
        {alerts.length === 0 ? (
          ...
          alerts.map((alert) => (
            <div 
              ...
              className={`border-l-2 ${alert.risk_level > 3 ? 'border-radar-red shadow-lg shadow-radar-red/5' : 'border-radar-cyan shadow-lg shadow-radar-cyan/5'} bg-radar-panel/30 p-3 relative group cursor-pointer hover:bg-radar-panel/50 transition-all overflow-hidden`}
            >
              <div className="flex justify-between items-start mb-1">
                ...
                {expandedId === alert.id && <span className={`text-[8px] ${alert.risk_level > 3 ? 'bg-radar-red' : 'bg-radar-cyan'} text-white px-1 font-bold`}>ANALYSIS ACTIVE</span>}
              </div>
              ...
              {expandedId === alert.id && (
                <div className="mt-3 pt-3 border-t border-radar-border/50 space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  <div className={`bg-radar-panel/60 p-2 border ${alert.risk_level > 3 ? 'border-radar-red/20 text-radar-red/90' : 'border-radar-cyan/20 text-radar-cyan/90'} italic text-[11px] leading-relaxed`}>
                    {alert.risk_level > 3 
                      ...
                      : "Status update: General monitoring indicates steady patterns with minor tactical adjustments. No immediate escalation predicted."}
                  </div>
                </div>
              )}
              ...
              {!expandedId && (
                <div className="mt-2 flex gap-2">
                  <span className={`text-[9px] ${alert.risk_level > 3 ? 'bg-radar-red/20 text-radar-red border-radar-red/30' : 'bg-radar-cyan/20 text-radar-cyan border-radar-cyan/30'} px-1 rounded border uppercase font-bold`}>
                    {alert.source}
                  </span>
                </div>
              )}
